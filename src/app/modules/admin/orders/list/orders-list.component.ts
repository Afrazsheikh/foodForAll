...
import { Donations, DonationsPagination } from 'app/modules/admin/orders/orders.types';
import { OrdersService } from 'app/modules/admin/orders/orders.service';
import { tap } from 'rxjs/operators';
import { activities } from 'app/mock-api/pages/activities/data';
import { Volunteers } from '../../volunteers/volunteers.types';
...
export class OrdersListComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.closeDetails();
                    this.isLoading = true;
                    return this._ordersService.getOrders(0, 10, 'pickupAddress', 'asc', query);
                }),
                map(() => {
        ...
        this._volunteersService.volunteers$
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((volunteers: Volunteers[]) => {
        ...
        if (this._sort && this._paginator) {
            ...
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    this.closeDetails();
                    this.isLoading = true;
                    return this._ordersService.getOrders(this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction);
                }),
                map(() => {
                    this.isLoading = false;
                })
    ...
    toggleDetails(orderId: string): void {
        ...
        // Get the order by id
        this._ordersService.getOrderById(orderId)
            .subscribe((order) => {
                ...
                this.selectedOrderForm.patchValue({ assignedVolunteerId: parseInt(order.volunteer['id']) })
                // Mark for check
                this._changeDetectorRef.markForCheck();
            });
    }
    ...
    updateSelectedOrder(): void {
        ...
        this._ordersService.updateOrder(order.id, order.assignedVolunteerId).subscribe(() => {
