    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                // Clone the products
                let canopies: any[] | null = cloneDeep(this._products);
                ...
                {
                    canopies.sort((a, b) => order === 'asc' ? a[sort] - b[sort] : b[sort] - a[sort]);
                }
                ...
                {
                    // Filter the products
                    canopies = canopies.filter(contact => contact.name && contact.name.toLowerCase().includes(search.toLowerCase()));
                }

                // Paginate - Start
                const canopiesLength = canopies.length;

                // Calculate pagination details
                const begin = page * size;
                const end = Math.min((size * (page + 1)), canopiesLength);
                const lastPage = Math.max(Math.ceil(canopiesLength / size), 1);
                ...
                else
                {
                    ...
                    canopies = canopies.slice(begin, end);

                    // Prepare the pagination mock-api
                    pagination = {
                        length    : canopiesLength,
                        size      : size,
                        page      : page,
                        lastPage  : lastPage,
                        startIndex: begin,
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                const product = cloneDeep(request.body.canopy);
                // Prepare the updated product
                let updatedCanopy = null;
                ...
                this._products.forEach((item, index, products) => {
                    ...
                    if ( item.id === id )
                    {
                        ...
                        updatedCanopy = products[index];
                    }
                });
                console.log(updatedCanopy)
                // Return the response
                return [200, updatedCanopy];
            });
